...
class TrackMark {
    constructor(x, y, angle) {
        ...
        this.y = y;
        this.angle = angle;
        this.life = 120; // Shortened to 2 seconds for high performance
        this.maxLife = 120;
    }
    ...
    draw(ctx) {
        ...
        ctx.fillStyle = `rgba(15, 23, 42, ${0.35 * (this.life / this.maxLife)})`;
...
function addTrackMark(tm) {
    trackMarks.push(tm);
    if (trackMarks.length > 30) trackMarks.shift();
}
...
function createMuzzleFlash(x, y, angle, isPlayer = true) {
    ...
    for (let i = 0; i < 6; i++) {
        ...
        addParticle(new Particle(x, y, color, vx, vy, Math.random() * 4 + 2, 12, 'circle'));
    }
    // Smoke
    for (let i = 0; i < 3; i++) {
        const vx = Math.cos(angle) * Math.random() * 2;
        const vy = Math.sin(angle) * Math.random() * 2;
        addParticle(new Particle(x, y, '#64748b', vx, vy, Math.random() * 6 + 4, 18, 'circle'));
    }
}
...
function createSparks(x, y) {
    ...
    for (let i = 0; i < 8; i++) {
        ...
        addParticle(new Particle(x, y, '#facc15', vx, vy, Math.random() * 2.5 + 1, 14, 'circle'));
    }
}
...
function createExpireEffect(x, y) {
    ...
    for (let i = 0; i < 6; i++) {
        ...
        const vx = Math.cos(angle) * speed;
        const vy = Math.sin(angle) * speed;
        addParticle(new Particle(x, y, '#94a3b8', vx, vy, Math.random() * 3 + 2, 15, 'circle'));
    }
}
...
function createExplosion(x, y, isBig = true) {
    ...
    // Shockwave ring
    addParticle(new Particle(x, y, '#ffaa00', 0, 0, 40, 22, 'ring'));

    // Fire particles
    const fireColors = ['#ff2200', '#ff6600', '#ffcc00', '#ffffff'];
    const count = isBig ? 35 : 18;
    ...
    for (let i = 0; i < count; i++) {
        ...
        addParticle(new Particle(x, y, color, vx, vy, size, Math.random() * 25 + 20, Math.random() > 0.3 ? 'circle' : 'rect'));
    }
    ...
    for (let i = 0; i < 12; i++) {
        ...
        const vx = Math.cos(angle) * speed;
        const vy = Math.sin(angle) * speed;
        addParticle(new Particle(x, y, '#334155', vx, vy, Math.random() * 12 + 6, 40, 'circle'));
    }
}
